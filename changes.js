...
In the example, adding 500 large strings to an array used up 496 MB of memory. In general, your goal should be to minimize memory usage, and 496 MB is pretty high for an app that builds strings.
...
Deprecated feature used StorageType.persistent is deprecated. Please use standardised navigator.storage instead. 1 source invisible.js:1 ==> design decision to avoid this : seting objects in a function or object and not in the root
Our answer

Several warnings will appear, Forced reflow is a likely performance bottleneck, Long task took x ms, and Handler took x ms.

The errors can be resolved by separating the calculation and application of the positioning using two separate for loops. The CSS can also be modified to use modern positioning, like flex instead of relative and absolute positioning.

The warnings that appear are due to the way the element positions are calculated and applied to the elements that move using window.getComputedStyle(), and slightly due to the elements that change back and forth from relative to absolute positioning. The changes are calculated and applied within the same for loop, which can be optimized by first calculating the changes, then applying the changes separately, because the operation is synchronous.

Why this response?

This is a good answer because it includes a list of the types of warnings that appear, an explanation of why those errors appear, and a discussion on how you could consider optimizing the code.
